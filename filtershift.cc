...
\n\t\t\t incramented between acquisitions\
...
\n\t\t\t '1' in the first row means that slice 1 was aquired\
\n\t\t\t first. '20' in the second row means that slice 20 was\
\n\t\t\t acquired 2nd. If present, all interelave parameters\
...
Option<float> lpf(string("--lpf"),0,string(
	 "\t Set the cutoff frequency of the lowpass filter in Hz.\
\n\t\t\t Note that by default, this is set to include 80% of\
\n\t\t\t the total availible bandwidth.\n"),
				  false,requires_argument);
...
\n\t\t\t that slice 1 was aquired first, and will be shifted 0\
\n\t\t\t seconds. '0.5' in the second row means that slice 2\
\n\t\t\t was acquired 0.5 seconds after the first slice, and\
\n\t\t\t will be shifted 0.5s. If present, all interelave\
